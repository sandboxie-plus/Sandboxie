...
CCodeEdit::CCodeEdit(QSyntaxHighlighter* pHighlighter, QWidget* pParent)
 : QWidget(pParent), m_pCompleter(nullptr), m_lastWordStart(-1), m_lastWordEnd(-1), m_caseCorrectionInProgress(false), m_completionInsertInProgress(false)
{
...
bool CCodeEdit::HandleBackspaceForCompletion(QKeyEvent* keyEvent)
{
	if (m_pCompleter && keyEvent->key() == Qt::Key_Backspace) {
		...
		if (!m_lastWrongWord.isEmpty() && m_lastWordStart >= 0 && m_lastWordEnd >= 0) {
			...
			// If backspace would affect the tracked word range, clear the tracking
			if (currentPos > m_lastWordStart && currentPos <= m_lastWordEnd) {
				ClearCaseCorrectionTracking();
			}
		}
		
		// Schedule completion update after backspace is processed (only in FullAuto mode)
		if (ShouldTriggerAutoCompletion()) {
			ScheduleWithDelay(10, [this]() {
				if (!m_pCompleter) return; // Additional safety check
		...
		return false; // Don't consume the event - let backspace be processed normally
	}
	return false;
}
