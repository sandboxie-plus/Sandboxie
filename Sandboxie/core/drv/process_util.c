...
static BOOLEAN Process_AddPath_2(
    ...
    BOOLEAN RemoveBackslashes, BOOLEAN CheckReparse, ULONG Level);
    ...
    BOOLEAN AddFirst, const WCHAR *value, BOOLEAN AddStar)
{
    ...
    ok = Process_AddPath_2(proc, list, value, setting_name,
               AddFirst, AddStar, RemoveBackslashes, CheckReparse, Level);
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (delay_ms > 0) {
                    ...
                    LARGE_INTEGER time;
                    time.QuadPart = -(SECONDS(1) / 2); // wait half a second = 500ms
                    KeDelayExecutionThread(KernelMode, FALSE, &time);
